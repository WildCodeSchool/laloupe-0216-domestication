...
class domestication_Contact_Info extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'domestication_contact_info_widget', 'description' => __( 'Display your contact info', 'domestication') );
        parent::__construct(false, $name = __('domestication: Contact info', 'domestication'), $widget_ops);
		$this->alt_option_name = 'domestication_contact_info';	
    }
	...
	<p><label for="<?php echo $this->get_field_id( 'address' ); ?>"><?php _e( 'Enter your address', 'domestication' ); ?></label>
	...
	<p><label for="<?php echo $this->get_field_id( 'phone' ); ?>"><?php _e( 'Enter your phone number', 'domestication' ); ?></label>
	...
	<p><label for="<?php echo $this->get_field_id( 'email' ); ?>"><?php _e( 'Enter your email address', 'domestication' ); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['domestication_contact_info']) )
			delete_option('domestication_contact_info');		  
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'domestication_contact_info', 'widget' );
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'domestication_contact_info', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
